...
async def get_user_org_role(
    ...
    Returns (organization, role) where role is 'owner', 'admin', 'editor', or 'viewer'.
    Raises 403 if not a member.

    DEPRECATED: Use require_permission() dependency instead for authorization.
    Kept for backward compatibility and utility queries.
    """
    # Get organization
...
async def invite_member(
    ...
    current_user: User = Depends(require_permission("team.invite_member")),
    db: AsyncSession = Depends(get_db),
):
    """
    Invite a new member to organization.
    Requires team.invite_member permission.
    """
    # Find user by email
...
async def list_members(
    ...
    role: str = Query(None, description="Filter by role (owner/admin/editor/viewer)"),
    current_user: User = Depends(require_permission("team.view_members")),
    db: AsyncSession = Depends(get_db),
):
    """
    List all members of an organization with search and filter.
    Requires team.view_members permission.
    ...
    if role:
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Invalid role: {role}. Must be 'owner', 'admin', 'editor', or 'viewer'"
            )
    ...
    if not owner_in_members:
        ...
        if owner_user:
            ...
            # Check if owner matches role filter
            matches_role = True
...
async def remove_member(
    member_id: UUID,
    current_user: User = Depends(require_permission("team.remove_member")),
    db: AsyncSession = Depends(get_db),
):
    """
...
async def update_member_role(
    ...
    current_user: User = Depends(require_permission("team.change_roles")),
    db: AsyncSession = Depends(get_db),
):
    """
    Update member's role in organization.
    Requires team.change_roles permission.
